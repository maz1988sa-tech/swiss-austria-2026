...
(function () {
  ...
  var L = {
    ...
    warnSheet: ['\u26A0 هذا شيت مختلف عن الشيت الذي يعمل عليه الفريق.',
      '\u26A0 This is a different sheet from the one the team is working on.'],
    warnDrop: ['\u26A0 هذا التحديث سيُنزل عدد المرشحين من {p} إلى {n}.',
      '\u26A0 This refresh would drop the candidates from {p} to {n}.'],
    warnGo: ['هل تريد المتابعة واستبدال البيانات الحالية؟', 'Continue and replace the current data?'],
    warnStop: ['أُلغي التحديث — لم تتغيّر البيانات', 'Refresh cancelled — nothing changed'],
  ...
  var TEAM_FP = '';       /* بصمة الشيت الذي يعمل عليه الفريق — تمنع السحب من شيت خاطئ */
  ...
  /* الحالات المبصومة = المطابقة حاليًا + المعلّقة التي لم يظهر أصحابها بعد */
  function mergeOv() {
    var o = hashOv(OVERRIDES);
    for (var k in (OVERRIDES_H || {})) if (!(k in o)) o[k] = OVERRIDES_H[k];
    return o;
  }
  ...
  function resolveOv() {
    ...
    for (var i = 0; i < DATA.length; i++) {
      var h = hk(DATA[i].key), v = OVERRIDES_H[h];
      if (v) { OVERRIDES[DATA[i].key] = v; delete OVERRIDES_H[h]; }
    }
    /* ما لم يطابق يبقى معلّقًا — قد يعود صاحبه في التحديث القادم، فلا نُسقطه */
    for (var k in OVERRIDES_H) return;
    OVERRIDES_H = null;
  }
  ...
  function saveWs(opts) {
    ...
    var ws = {
      ...
      overrides: mergeOv(), ovh: 1, sheetFp: hk(GSheet.sheetIdOf(SHEET_URL)),
    ...
    return Store.saveWorkspace(ws, opts).then(function (res) {
      ...
      TEAM_FP = ws.sheetFp || TEAM_FP;   /* هذا الشيت صار شيت الفريق المعتمد */
  ...
  function sync(silent) {
    ...
    return GSheet.load(url, function (phase, info) {
      ...
      /* ===== حاجز أمان قبل استبدال البيانات =====
         سببه واقعة حقيقية: لصق رابط شيت قديم استبدل 11,031 مرشحًا بـ67 بصمت.
         نعترض على حالتين: شيت غير شيت الفريق، أو انهيار مفاجئ في العدد. */
      var warn = [];
      var fp = hk(GSheet.sheetIdOf(url));
      if (TEAM_FP && fp !== TEAM_FP) warn.push(T('warnSheet'));
      if (prev && prev.total >= 200 && res.records.length < prev.total * 0.6)
        warn.push(T('warnDrop', { p: nf(prev.total), n: nf(res.records.length) }));
      if (warn.length && !window.confirm(warn.join('\n\n') + '\n\n' + T('warnGo'))) {
        paint(log + stepRow(2, T('warnStop'), ''));
        setSyncing(false); toast(T('warnStop'));
        return;                                  /* البيانات الحالية كما هي */
      }
